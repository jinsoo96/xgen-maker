...
<div class="tab on" id="tab-run">
 <div id="runcols">
  <div id="logcol"><div id="log"><div class="ev info">쿼리를 입력하면 MAKER 루프가 돕니다. 진행 로그가 실시간으로 흐르고, 결과가 아래에 뜹니다.</div></div></div>
  <aside id="side">
   <div id="runstate"><span class="spin"></span><span id="runstate-t">실행 중…</span></div>
   <div class="side-h">진행 단계</div>
   <div id="gates"></div>
   <div class="side-h" style="margin-top:14px">🎯 고칠 코드 위치 <span class=muted id="landn"></span></div>
   <div id="landing"><div class=muted style="font-size:12px">쿼리를 실행하면 코드에서 관련 위치를 찾아 여기 보여줍니다.</div></div>
  </aside>
 </div>
</div>
...
<div class="tab" id="tab-tests"><div class="muted">불러오는 중…</div></div>
<div class="tab" id="tab-ui"><div class="muted">불러오는 중…</div></div>
<div class="tab" id="tab-deploy"><div class="muted">불러오는 중…</div></div>
<div class="tab" id="tab-login"><div class="muted">불러오는 중…</div></div>
...
document.getElementById('newsess').onclick=()=>{
 ...
 document.getElementById('log').innerHTML='<div class="ev info">새 세션. 쿼리를 입력하면 MAKER 루프가 돕니다. 진행 로그가 실시간으로 흐르고, 결과가 아래에 뜹니다.</div>';
 document.getElementById('gates').innerHTML='';
 document.getElementById('landing').innerHTML='<div class=muted style="font-size:12px">쿼리를 실행하면 MAKER가 지식그래프에서 찾은 코드 위치가 여기 뜹니다.</div>';
...
function drawGraph(data,svg,tip,onPick){
 ...
 if(!data.nodes||!data.nodes.length){svg.innerHTML='<text x="50%" y="50%" text-anchor=middle fill="#889" font-size=13>'+esc(data.reason||'결과 없음')+'</text>';return;}
 ...
 svg.onmousemove=ev=>{const g=ev.target.closest('.gn');
  ...
  if(g&&tip){const n=nodes[+g.dataset.i];
   tip.innerHTML=isRepo
    ?`<b>${esc(n.name)}</b><br><span class=muted>코드 ${(n.count||0).toLocaleString()}개 · 다른 레포와 ${n.deg||0}개 연결</span><br><span style="color:var(--primary)">클릭 → 이 레포 내부 그래프</span>`
    :`<b>${esc(n.name)}</b> <span class=muted>${esc(n.kind)} · 연결 ${n.deg||0}</span><br><span class=muted>${esc(n.repo)} · ${esc(n.path)||'-'}</span>${n.deprecated?'<br><span style="color:var(--ember)">⚠ 착지 회피</span>':''}${n.note?'<br>📝 '+esc(n.note):''}<br><span style="color:var(--primary)">클릭 → 코드 보기</span>`;
...
function openNodeEditor(n,ged){
 ...
 ged.innerHTML=`<div class=geh><b>${esc(n.name)}</b> <span class=muted>${esc(n.kind)}</span><span class=gex>✕</span></div>`+
  ...
  `<label class=gel>작업 메모<textarea id=gnote rows=2 placeholder="예: 레거시 — 신규 작업은 X 모듈로">${esc(n.note||'')}</textarea></label>`+
  `<label class=gel>요약 교체<input id=gsum value="${esc(n.summary||'')}" placeholder="사람이 쓴 요약(검색·판단에 사용)"></label>`+
  `<label class=gelc><input type=checkbox id=gdep ${n.deprecated?'checked':''}> 착지 회피(deprecated) — 이 코드로 자동 착지하지 않음</label>`+
 ...
 document.getElementById('gsave').onclick=()=>{
  ...
  const out=document.getElementById('gsaveout');out.textContent='저장 중…';
  jget('/api/annotate?'+p.toString()).then(r=>{
   if(r.ok){out.innerHTML='<span style="color:var(--success)">✓ 저장됨</span> (overlay 영속)';
...
function loadNodeCode(n,gcode){
 ...
 gcode.innerHTML='<div class=muted style="font-size:12px">코드 불러오는 중… '+esc(n.name)+'</div>';
 jget('/api/node-code?id='+encodeURIComponent(n.id)).then(r=>{
  if(!r.ok){gcode.innerHTML='<div class=gch><b>'+esc(n.name)+'</b> <span class=muted>'+esc(n.kind)+'</span></div><div class=muted style="font-size:12px">코드 없음: '+esc(r.error)+'</div>';return;}
  ...
  gcode.querySelector('.gcwork').onclick=(ev)=>{const b=ev.currentTarget;
   ...
   line('step','작업 대상: '+b.dataset.nm+' — 무엇을 바꿀지 이어서 쓰고 실행','▶');};
 }).catch(e=>{gcode.innerHTML='<div class=muted>코드 조회 실패: '+esc(e.message||e)+'</div>';});
}
function render(t){
 ...
 if(t==='graph') jget('/api/graph').then(d=>{
  ...
  h+='<div id=glegend>'+Object.keys(KCOLOR).map(k=>`<span class=lg><i style="background:${KCOLOR[k]}"></i>${k}</span>`).join('')+'<span class=muted style="margin-left:8px">레포 클릭=내부로 · 코드노드 클릭=코드 · 휠=확대/축소 · 빈곳 끌기=이동 · 선 굵기=연결 많음</span></div>';
  h+='<div id=gcode></div>';
  ...
  h+='<h4>레포별 (마지막 동기화 지점) <button id=gsync class="ghost" style="font-size:12px;padding:4px 10px;margin-left:6px">⟳ 지금 동기화</button></h4>'+
  ...
  h+='<h4>노드 종류 분포</h4><table><tr><th>종류</th><th>개수</th></tr>'+
  ...
  document.getElementById('gsync').onclick=()=>{const o=document.getElementById('gsyncout');
   o.innerHTML='<span class=spin style="display:inline-block;width:13px;height:13px;vertical-align:middle"></span> 최신 코드로 증분 동기화 중…';
   jget('/api/sync').then(s=>{
    if(!s.ok){o.innerHTML='<span style="color:var(--danger)">✗ '+esc(s.error)+'</span>';return;}
    const pr=(s.per_repo||[]).map(r=>`${esc(r.repo||'?')}: ${r.action==='full_rebuild_needed'?'<span style="color:var(--ember)">재빌드 필요</span>':(r.changed||0)+'개'}`).join(' · ');
  ...
  // 1단계 — 레포 간 그래프(즉시)
  const showRepos=()=>{ged.style.display='none';gcode.style.display='none';busy('레포 관계 불러오는 중…');
   setCrumb([{t:'전체 레포'}]);
   jget('/api/repo-graph').then(draw).catch(e=>draw({reason:'조회 실패: '+(e.message||e)}));};
  // 2단계 — 한 레포 내부
  const showRepo=(repo)=>{ged.style.display='none';gcode.style.display='none';busy(repo+' 내부 펼치는 중…');
   setCrumb([{t:'전체 레포',lv:'repo'},{t:repo}]);
   jget('/api/subgraph?repo='+encodeURIComponent(repo)+'&n='+gn.value).then(draw).catch(e=>draw({reason:'조회 실패: '+(e.message||e)}));};
  window.__showRepo=showRepo;
  // 검색 — 레포 무관 코드 검색
  const search=()=>{const q=gq.value.trim();if(!q){showRepos();return;}
   ged.style.display='none';busy('검색 중…');setCrumb([{t:'전체 레포',lv:'repo'},{t:'검색: '+q}]);
   jget('/api/subgraph?n='+gn.value+'&q='+encodeURIComponent(q)).then(draw).catch(e=>draw({reason:'조회 실패: '+(e.message||e)}));};
 ...
 if(t==='history') jget('/api/history').then(d=>{
  el.innerHTML='<h3>작업 이력 <span class=muted>(줄을 누르면 단계별 진행·요약·되돌리기)</span></h3>'+
   ...
   d.sessions.map(s=>`<tr class="hrow" data-sid="${esc(s.session)}"><td><span class="badge ${cls(s.outcome)}">${esc(s.outcome)}</span></td><td>${esc(s.query).slice(0,52)}</td><td class="muted">${esc(s.branch)}</td><td>${s.mr?`<a href="${esc(s.mr)}" target=_blank onclick="event.stopPropagation()">MR</a>`:''}</td></tr>`).join('')+'</table></div>'+
 ...
 if(t==='branches'){
  ...
  el.innerHTML='<h3>브랜치 / 릴리즈 / 활동 <span class=muted>(작업 브랜치·승격 경로 + 누가 뭘 고쳤나)</span></h3>'+sel+'<div id="bbody" class=muted style="margin-top:12px">불러오는 중…</div>'+
   '<h4 style="margin-top:16px">활동 검색 <span class=muted>(커밋 메시지·작성자로 검색 — 누가 뭘 고쳤나)</span></h4>'+
   '<div class=gsearch><input id=actq placeholder="예: governance, 로그인, 특정 작성자" autocomplete=off><button id=actbtn>검색</button></div><div id="actbody" class=muted style="font-size:12px">검색어를 넣거나 비우고 검색하면 최근 커밋을 보여줍니다.</div>';
  const load=()=>{const repo=document.getElementById('brepo').value; const bb=document.getElementById('bbody');
   ...
   Promise.all([fetch('/api/branches?repo='+encodeURIComponent(repo)).then(r=>r.json()),
    ...
    const brow=x=>`<tr class=brow data-b="${esc(x.name)}" title="클릭 → 이 브랜치 활동 검색"><td>${esc(x.name)}</td><td class=muted>${esc(x.author||'')}</td><td class=muted>${esc((x.when||'').slice(0,10))}</td></tr>`;
    let h='<h4>작업 브랜치(최근) <span class=muted style="font-weight:400">— 줄 클릭 시 아래 활동 검색</span></h4><table><tr><th>브랜치</th><th>작성자</th><th>날짜</th></tr>'+((b.work_recent||[]).map(brow).join('')||'<tr><td colspan=3 class=muted>없음</td></tr>')+'</table>';
  ...
  const searchAct=()=>{const repo=document.getElementById('brepo').value;const q=document.getElementById('actq').value.trim();const ab=document.getElementById('actbody');
   ...
   fetch('/api/activity?repo='+encodeURIComponent(repo)+'&q='+encodeURIComponent(q)).then(r=>r.json()).then(a=>{
    if(a.error){ab.innerHTML='<div class=muted>조회 실패: '+esc(a.error)+'</div>';return;}
    if(!a.commits.length){ab.innerHTML='<div class=muted>결과 없음'+(q?' ("'+esc(q)+'")':'')+'</div>';return;}
  ...
  document.getElementById('brepo').onchange=()=>{load();document.getElementById('actbody').textContent='검색어를 넣거나 비우고 검색하면 최근 커밋을 보여줍니다.';};
 ...
 if(t==='tests') jget('/api/tests').then(d=>{
  let h='<h3>테스트 · 검증 이력 <span class=muted>(수렴 루프의 sandbox·py_compile·pytest·회귀·품질judge 결과)</span></h3>';
 ...
 if(t==='ui') jget('/api/ui-status').then(d=>{
  ...
  let h='<h3>화면 검증 <span class=muted>(바뀐 화면을 기준과 픽셀 비교 — R11·R23)</span></h3>';
  h+='<table><tr><th>프리뷰 주소</th><td>'+(d.preview_base?esc(d.preview_base)+' '+(d.reachable?'<span class="badge ok">도달</span>':'<span class="badge fail">미도달</span>'):'<span class=muted>config preview_base 미설정 — 아무 URL이나 아래에서 직접 검증 가능</span>')+'</td></tr>'+
   '<tr><th>픽셀 비교(Pillow)</th><td>'+yn(d.pillow)+'</td></tr><tr><th>스냅샷(Playwright)</th><td>'+yn(d.playwright)+'</td></tr></table>';
  h+='<div class="uiv"><div class=gsearch><input id=uiurl placeholder="검증할 화면 URL (예: http://localhost:3100/ 또는 이 대시보드 주소)" autocomplete=off>'+
   '<button id=uisnap>스냅샷+비교</button><button id=uibase class="ghost">기준으로 저장</button></div>'+
   '<div id=uiout class=muted style="font-size:12px">URL을 넣고 <b>스냅샷+비교</b>: 기준이 있으면 바뀐 정도(%)와 diff 이미지를, 없으면 현재 화면을 보여줍니다. <b>기준으로 저장</b>으로 현재 화면을 기준선으로 삼으세요.</div>'+
   '<div id=uishots></div></div>';
  if(d.baselines.length){h+='<h4>기준 화면(baseline)</h4><div class=gal>'+d.baselines.map(b=>`<figure><img src="${esc(b.url)}" loading=lazy><figcaption>${esc(b.slug)}</figcaption></figure>`).join('')+'</div>';}
  if(d.recent.length){h+='<h4>최근 검증 diff</h4><div class=gal>'+d.recent.map(r=>`<figure><img src="${esc(r.diff_url)}" loading=lazy><figcaption>${esc(r.route)} <span class=muted>${esc(r.session).slice(0,16)}</span></figcaption></figure>`).join('')+'</div>';}
  el.innerHTML=h;
  const uiurl=document.getElementById('uiurl'),uiout=document.getElementById('uiout'),uishots=document.getElementById('uishots');
  const run=(save)=>{const u=uiurl.value.trim();if(!u){uiout.textContent='URL을 입력하세요.';return;}
   uiout.innerHTML='<span class=spin style="display:inline-block;width:14px;height:14px;vertical-align:middle"></span> 스냅샷 캡처 중… (수 초~수십 초)';uishots.innerHTML='';
   jget('/api/ui-snap?url='+encodeURIComponent(u)+(save?'&baseline=1':'')).then(r=>{
    ...
    if(r.baseline_saved){uiout.innerHTML='<span style="color:var(--success)">✓ 기준으로 저장됨</span> — 다음부터 이 화면과 비교합니다.';uishots.innerHTML=`<figure><img src="${esc(r.snap_url)}?t=${Date.now()}"><figcaption>새 기준</figcaption></figure>`;loaded['ui']=0;return;}
    let msg='<span style="color:var(--success)">✓ 스냅샷 완료</span>';
    ...
    if(r.diff&&r.diff.diff_url)sh+='<figure><img src="'+esc(r.diff.diff_url)+'?t='+Date.now()+'"><figcaption>차이(diff)</figcaption></figure>';
 ...
 if(t==='login') jget('/api/auth').then(d=>{
  let h='<h3>로그인 · 자가검증 <span class=muted>(한 번 로그인하면 지속 — R24·R25)</span></h3>';
  h+='<h4>저장된 로그인</h4><table>'+
   `<tr><th>LLM 제공자</th><td>${esc(d.provider)} <span class=muted>(${esc(d.model)})</span></td></tr>`+
   `<tr><th>API 키</th><td>${d.api_key_set?'<span class="badge ok">설정됨</span>':'<span class=muted>구독 로그인(키 불필요)</span>'}</td></tr>`+
   `<tr><th>GitLab</th><td>${esc(d.gitlab_url)}${d.gitlab_user?' · '+esc(d.gitlab_user):''} ${d.gitlab_token_set?'<span class="badge ok">토큰 저장됨</span>':'<span class="badge fail">미로그인</span>'}</td></tr>`+
   `<tr><th>저장 위치</th><td class=muted style="font-family:Consolas,monospace">${esc(d.auth_file)} ${d.auth_file_exists?'':'<span class=muted>(없음 — 기본값)</span>'}</td></tr></table>`;
  h+='<div class=gsearch style="margin-top:12px"><button id=authchk>로그인 상태 확인</button><button id=docbtn class="ghost">자가검증(doctor) 실행</button></div>';
  ...
  document.getElementById('authchk').onclick=()=>{const o=document.getElementById('authout');
   o.innerHTML='<span class=spin style="display:inline-block;width:14px;height:14px;vertical-align:middle"></span> 확인 중… (claude CLI·GitLab 호출, 최대 1~2분)';
   jget('/api/auth-check').then(r=>{
    const cl=r.claude,gl=r.gitlab;
    o.innerHTML='<table><tr><th>Claude</th><td>'+(cl.authenticated?'<span class="badge ok">✓ 로그인 유지됨</span>':'<span class="badge fail">✗ 미인증</span> <span class=muted>'+esc(cl.reason)+'</span>')+'</td></tr>'+
  ...
  document.getElementById('docbtn').onclick=()=>{const p=document.getElementById('docout');
   p.style.display='block';p.textContent='자가검증 실행 중… (능력별 실제 동작 확인, 1~2분 걸릴 수 있음)';
   jget('/api/doctor').then(r=>{p.textContent=r.output||'(출력 없음)';}).catch(e=>{p.textContent='실패: '+(e.message||e);});};
  }).catch(tabErr(el));
...
function resetPanel(){
 ...
 document.getElementById('landing').innerHTML='<div class=muted style="font-size:12px">지식그래프 검색 중…</div>';
 document.getElementById('landn').textContent='';
 runstate(true,'시작…');
}
...
function showSession(sid,target){
 const d=target||document.getElementById('histdetail'); d.innerHTML='<div class=muted>불러오는 중…</div>';
 jget('/api/session?id='+encodeURIComponent(sid)).then(s=>{
  let h=`<h4>${esc(s.query)||'(쿼리 없음)'}</h4><div class=muted style="margin-bottom:10px">결과 <span class="badge ${cls(s.outcome)}">${esc(s.outcome)}</span>${s.mr?` · <a href="${esc(s.mr)}" target=_blank>MR</a>`:''}</div>`;
  ...
  if(s.undoable) h+=`<div class="side-h" style="margin-top:12px">되돌리기</div><div class=muted style="font-size:12px;margin-bottom:6px">이 세션이 만든 브랜치(${esc(s.branch)})를 삭제합니다. ${s.pushed?'원격에도 올라간 상태 — 원격 삭제는 아래 체크':'로컬만 존재'}.</div>`+
   `<label style="font-size:12px"><input type=checkbox class="undoremote" ${s.pushed?'':'disabled'}> 원격 브랜치도 삭제(외부 반영)</label><br><button class="undobtn danger" data-sid="${esc(s.session)}">↺ 되돌리기</button><div class="undoout muted" style="font-size:12px;margin-top:6px"></div>`;
  else h+='<div class=muted style="font-size:12px;margin-top:12px">되돌릴 브랜치가 없는 작업(질문형 등)입니다.</div>';
  d.innerHTML=h;
  ...
  if(rb) rb.onclick=()=>{document.querySelector('nav button[data-t=run]').click();
   ...
   line('step','이어가기: '+rb.dataset.q+' (실행을 누르면 최신 코드로 다시 진행)','↻');};
 }).catch(e=>{d.innerHTML='<div class=muted>세션 불러오기 실패: '+esc(e.message||e)+'</div>';});
}
function doUndo(sid,remote,panel){
 const out=(panel||document).querySelector('.undoout');
 if(!confirm('정말 이 작업을 되돌릴까요? 브랜치가 삭제됩니다'+(remote?' (원격 포함)':'')+'.')){return;}
 out.textContent='되돌리는 중…';
...
def _run_query(config: MakerConfig, graph: Graph, query: str, q: queue.Queue,
    ...
    try:
        ...
        q.put({"type": "stopped", "message": "사용자가 실행을 중지했습니다"})
...
class MakerWebHandler(BaseHTTPRequestHandler):
    ...
    def _graph_status(self) -> dict:
        ...
        for _ in range(5):
            try:
                ...
                time.sleep(0.02)
        return {"nodes": len(self.graph.nodes), "edges": 0, "by_kind": [], "repos": []}
    ...
    def _repo_graph(self) -> dict:
        ...
        nodes = [{"id": r, "name": r, "kind": "repo", "repo": r, "path": "",
                  ...
                  "summary": f"{c:,}개 코드 노드 · 다른 레포와 {deg.get(r, 0)}개 연결"}
                 for r, c in sorted(counts.items(), key=lambda kv: -kv[1])]
        edges = [{"src": a, "dst": b, "kind": "cross", "weight": w} for (a, b), w in pair.items()]
    ...
    def _repo_subgraph(self, repo: str, max_nodes: int = 160) -> dict:
        ...
        adj = self._adjacency()
        inside = [nid for nid, n in g.nodes.items() if n["repo"] == repo]
        if not inside:
            return {"nodes": [], "edges": [], "reason": f"'{repo}' 레포 노드 없음"}
        inside.sort(key=lambda nid: -len(adj.get(nid, ())))
        keep = set(inside[:max_nodes])
        nodes = [{"id": nid, "name": g.nodes[nid]["name"], "kind": g.nodes[nid]["kind"],
                  "repo": g.nodes[nid]["repo"], "path": g.nodes[nid].get("path", ""),
                  "seed": False, "deg": len(adj.get(nid, ())),
                  "note": g.nodes[nid].get("meta", {}).get("note", ""),
                  "deprecated": bool(g.nodes[nid].get("meta", {}).get("deprecated")),
                  "summary": g.nodes[nid].get("meta", {}).get("summary", "")}
                 for nid in keep]
        edges = [{"src": e["src"], "dst": e["dst"], "kind": e["kind"]}
                 for e in g.edges if e["src"] in keep and e["dst"] in keep]
    ...
    def _subgraph(self, query: str, max_nodes: int = 320) -> dict:
        ...
        if query:
            ...
            if not hits:
                return {"nodes": [], "edges": [], "reason": "일치 노드 없음 — 코드 심볼로 검색(예: config, router, Service)"}
            ...
            seeds = [nid for nid, _ in sorted(deg.items(), key=lambda kv: -kv[1])[:14] if nid in g.nodes]
            if not seeds:
                return {"nodes": [], "edges": [], "reason": "그래프가 비어있음 — ⟳ Sync로 코드 동기화"}
        adj = self._adjacency()
        ...
        nodes = [{"id": nid, "name": g.nodes[nid]["name"], "kind": g.nodes[nid]["kind"],
                  ...
                  "deprecated": bool(g.nodes[nid].get("meta", {}).get("deprecated")),
                  "summary": g.nodes[nid].get("meta", {}).get("summary", "")}
                 for nid in keep if nid in g.nodes]
        edges = [{"src": e["src"], "dst": e["dst"], "kind": e["kind"]}
                 for e in g.edges if e["src"] in keep and e["dst"] in keep]
    ...
    def _node_code(self, node_id: str) -> dict:
        ...
        if not n:
            return {"ok": False, "error": "노드 없음"}
        repo_path = self.config.repos.get(n.get("repo", ""))
        if not repo_path:
            return {"ok": False, "error": f"레포 '{n.get('repo')}' 경로 미설정(config repos)"}
        rel = n.get("path", "")
        if not rel:
    ...
    def _auth_check(self) -> dict:
        ...
        if a.gitlab_token:
            ...
            out["gitlab"] = {"ok": False, "reason": "미로그인 — maker login --gitlab-user/-password",
                             "url": a.gitlab_url}
        return out
    ...
    def _ui_snap(self, qs: dict) -> dict:
        ...
        if not res.get("ok"):
            return {"ok": False, "error": "스냅샷 실패: " + res.get("reason", "")[-200:]}
    ...
    def _annotate(self, qs: dict) -> dict:
        ...
        if not kw:
            return {"ok": False, "error": "변경 내용 없음"}
        overlay_path = Path(self.config.kg_path).parent / "overlay.json"
        edits = annotate(overlay_path, node, **kw)
        # 살아있는 그래프에 즉시 반영(재빌드/새로고침 없이 바로 보이게)
        meta = self.graph.nodes[node].setdefault("meta", {})
        for k, v in kw.items():
            meta[k] = v
        if "summary" in kw:
    ...
    def _undo(self, qs: dict) -> dict:
        ...
        if not act:
            return {"ok": False, "errors": ["되돌릴 브랜치가 없는 세션(질문형 등)"]}
    ...
    def _route(self):
        ...
        if parsed.path == "/":
            ...
            if ev:
                ...
                self._json({"ok": False, "error": "실행 없음(이미 끝났거나 잘못된 id)"})
