...
# 클래스) 길다는 이유로 관련 있는 함수가 밀린다. 파일 노드가 참조 식별자·화면 문구까지
# 담게 되면서 편차는 더 커졌고, 길이로 깎을 이유는 그만큼 사라졌다.
# 값은 머지된 MR 2,412건(처음 보는 것만)으로 두 바퀴 훑어 정했다 —
#   0.0 R@10 0.807 MRR 0.572 ← 채택 · 0.05 0.806/0.562 · 0.1 0.796/0.559 ·
#   0.15 0.795/0.553 · 0.2 0.786/0.548
# ⚠️ 256건 표본에서는 0.2가, 78건에서는 0.5가 최적으로 보였다. 둘 다 그 표본의
#    치우침이었다 — 작은 표본으로 이 값을 정하지 말 것.
_B = 0.0
...
# ⚠️ 이 값을 지표만 보고 올리지 말 것. 처음 보는 MR 2,412건에서 감점을 풀수록
# 전체 R@1이 오른다(0.35→0.458 · 0.6→0.465 · 0.8→0.467). 그런데 그건 벤치마크가
# "그 MR이 바꾼 파일 중 아무거나" 찾으면 맞춘 것으로 세기 때문이다 — 감점을 풀면
# 테스트 파일을 더 잘 찾고 그게 점수가 된다. 정답에서 테스트를 빼고 구현 파일만
# 세면 정반대다: 0.35 R@1 0.455 R@10 0.806 · 0.6 0.454/0.804 · 0.8 0.448/0.804 ·
# 1.0 0.442/0.800. 즉 감점을 풀면 에이전트는 고칠 자리 대신 테스트에 착지한다.
...
_REPO_AFFINITY = 2.5
# 다시 하지 말 것: '저장소 크기 보정'은 표본 치우침을 메우던 값이었다.
# 큰 저장소가 '맞아서'가 아니라 '커서' 이긴다고 보고, 노드 수로 눌러 기회를 고르게
# 했다. 294건 표본에서는 R@1이 네 분할 모두 올랐다. 그런데 그 표본은 정답이 작은
# 저장소에 몰려 있었다 — 게이트웨이 14%·모델 13%인데 그 저장소들의 노드 비중은
# 1.0%·3.6%다. 보정은 정확히 그 치우침을 메우고 있었다.
# 처음 보는 MR 2,412건에서는 정답 분포가 노드 비중을 그대로 따르고(워크플로우
# 28% vs 35%, 코어 23% vs 14%), 보정은 α가 커질수록 최근·이전 양쪽에서 단조롭게
# 나빠진다(R@1 0.457 → 0.449 → 0.444). 메울 치우침이 없으면 그건 그냥 왜곡이다.
...
# 처음 보는 MR 2,412건에서도 2가 셋 다 최고다(1 0.807/0.565 · 2 0.809/0.573 ·
# 3 0.808/0.572).
_LABEL_WEIGHT = 2
